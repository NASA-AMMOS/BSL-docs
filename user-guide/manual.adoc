...
The Mock BPA has the option to parse and load ION-DTN JSON policy rules, with some notable differences/modifications, into the sample policy provider. The current differences are as follows:

`policyrule_set`::
 Each `policyrule` should be included in a JSON list, which is the value of an attribute `policyrule_set`. An example format of parsable JSON with two policy rules is as follows:
...
`key_wrap`::
 An extra security context parameter `id` `key_wrap` will be parsed. When the `value` is set to "0", key wrapping will be skipped. For all other values, key wrapping will be enabled. This addition is due to determination to key wrap being a required paramter for security operations when executed within the implementations of the Default Security Contexts.
