...
`policyrule_set`::
Each file's JSON encoding must have the attribute `policyrule_set`, which contains a JSON list of ION-like JSON-encoded `policyrule` as specified in <<ION-BPSec-Policy-Manual>>. An example with two policy rules is as follows:
