...
The BSL includes a simple rule-based example PP that may be utilized. This policy provider's data is mutex-protected and may be re-used amongst multiple threads / BSL contexts (see the Mock BPA for an example).
...
===== Sample policy provider initialization from JSON steps
