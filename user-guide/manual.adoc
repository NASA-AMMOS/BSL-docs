...
Both of the default contexts use the BSL frontend for abstracting cryptographic processing.
It is expected that alternative and/or future contexts will also use the BSL frontend for abstracting such processing, and that that frontend API will evolve as those needs change.
...
Initialize the backend cryptographic interface with `BSL_CryptoInit()`.
Then, register the two Default Security Contexts ("BIB-HMAC-SHA2" and "BCB-AES-GCM") with the Library Context.
