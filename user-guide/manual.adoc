...
The BSL includes a simple rule-based example PP that may be utilized by any BPA, and is used by the Mock BPA used for BSL testing (see <<sec-mockbpa>>). This policy provider's data is mutex-protected and may be re-used amongst multiple threads / BSL contexts (see the Mock BPA for an example).
...
. Register the policy with BSL by calling `BSL_API_RegisterPolicyProvider()`, the policy where the policy descriptor's user_data is set to the initialized policy provider.
