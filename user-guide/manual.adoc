...
. Heap memory management callbacks used by the BSL. If not provided, the standard `libc` functions for heap allocation are used.
...
plantuml::static/bpa-associations.puml[format=svg,width=60mm]
...
plantuml::static/bsl-breakdown.puml[format=svg,width=70mm]
...
plantuml::static/secop-associations.puml[format=svg,width=100mm]
...
plantuml::static/secondary-associations.puml[format=svg,width=50mm]
...
plantuml::static/bundle-workflow.puml[format=svg,width=100mm]

The BSL Frontend API consists of two primary functions for per-bundle operation: 

1. `BSL_API_QuerySecurity` covers steps 1 through 6 above. The function first utilizes the policy providers to query on a bundle. Next, each security operation in the resulting Action Set is validated using the security context associated with that security operation.

2. `BSL_API_ApplySecurity` covers steps 7 through 17 above. The function first executes each security operation with its associated security context. Next, the policy providers will finalize over the results.
...
[#sec-defaultsc]
== Example of Default Security Contexts

The BSL source and default build includes implementations of the two Default Security Contexts, both specified in <<RFC9173>>, as an working example of how to use the BSL frontend and crypto APIs.
...
`policy_action_on_fail`::
 A full policy action set implementation does not exist yet within BSL. For now, a temporary `policy_action_on_fail` attribute should be set to "delete_bundle", "drop_block", or "do_nothing".
...
[qanda]
Why is BSL writing logs to standard error (`stderr`)?::
