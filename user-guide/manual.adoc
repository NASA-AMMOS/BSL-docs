...
The BSL includes a simple rule-based example PP that may be utilized by any BPA (see <<sec-examplepp>>), and is used by the Mock BPA for BSL testing (see <<sec-mockbpa>>).
...
This implementation corresponds to the shared library `bsl_default_sc` as explained in the <<bsl-product-guide>> and the API Docs subsection on https://nasa-ammos.github.io/BSL/html/examples-and-mocks.html#example-default-scs[Example Default Security Contexts].
...
`BSLX_MAX_AES_PAD` = 64::
Maximum size of padding added to AES operation by crypto finalize operation.
AES-GCM will not produce extra padding, and this value is likely inconsequential.
...
The BSL source and default build includes a policy provider which has heritage in the ION BPA and is used to configure options for the Default Security Context implementations of <<sec-defaultsc>>.
...
This implementation corresponds to the shared library `bsl_example_pp` as explained in the <<bsl-product-guide>> and the API Docs subsection on https://nasa-ammos.github.io/BSL/html/examples-and-mocks.html#example-pps[Example Policy Providers].
...
Users may find it useful to reference the Mock BPA for a working example of library and bundle workflow, and working examples of initializing, registering, and operating the Default Security Context (of <<sec-defaultsc>>) and the Example Policy Provider (of <<sec-examplepp>>).
