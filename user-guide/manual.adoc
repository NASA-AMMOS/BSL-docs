...
plantuml::static/bsl-breakdown.puml[format=svg,width=60mm]
...
Heap Allocation::
For the dynamic backend, heap memory management callbacks are contained in the `BSL_DynMemHostDescriptors_t` struct.
...
Telemetry Counters::
Telemetry from each BSL context is accessed through the synchronous function `BSL_LibCtx_AccumulateTlmCounters()`.
...
plantuml::static/secondary-associations.puml[format=svg,width=60mm]
...
[#sec-defaultsc]
== Implementation of Default Security Contexts (RFC9173)

The BSL source and default build includes two Default Security Context implementations, both specified in <<RFC9173>>:
