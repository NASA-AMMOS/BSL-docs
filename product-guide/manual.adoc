...
+
Major files are installed under `/usr/lib64/` with sample policy schema under `/usr/share/bsl/sample_pp/`.
`bsl-devel`::
...
`/usr/share/`::
The OS-standard path for architecture-independent data files.
The BSL installs its data files under the `bsl` sub-directory.
`/usr/share/doc/`::
The OS-standard path for library and package documentation files.
The BSL installs its documentation files under the `bsl` sub-directory.
...
`BUILD_MOCK_BPA`:: Enable building the Mock BPA executable (default ON).
